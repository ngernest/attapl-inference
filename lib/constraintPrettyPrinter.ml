...
let active_mode mode = 
  match mode with
	...
	set_formatter_out_functions {
		...
		out_spaces = r.spaces;
		out_indent = fun n -> r.out (String.make n ' ') 0 n
	};
	if r.with_tags then (
	  set_formatter_stag_functions
	    {
	      mark_open_stag   = (fun t -> open_stag t; "");
	      mark_close_stag  = (fun t -> close_stag (); "");
	...
	set_formatter_out_functions {
		...
		out_newline = newline;
		out_spaces = spaces;
		out_indent = fun n -> outputf (String.make n ' ') 0 n
	}
	...
	set_formatter_out_functions {
		...
		out_indent = fun n -> 
			(fun s b e -> output_string out (String.sub s b e)) 
		  (String.make n ' ') 0 n
	}
